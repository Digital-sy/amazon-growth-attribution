...
# 环境变量沿用 /root/lingxing_env.sh：
# LINGXING_DB_HOST / PORT / USER / PASSWORD / NAME
...
# 基础表按店铺+UTC时间范围抓取，依赖该索引避免重复全表扫描。
index_exists="$(scalar "
SELECT COUNT(*)
...
START_MONTH="${ATTR_START_MONTH:-$(scalar "
SELECT DATE_FORMAT(MIN(report_date), '%Y-%m-01')
FROM lingxing_ad_spend_daily
WHERE ad_type IN ('SP','SD');
") }"
START_MONTH="${START_MONTH//[[:space:]]/}"

END_MONTH_EXCLUSIVE="${ATTR_END_MONTH_EXCLUSIVE:-$(scalar "
SELECT DATE_FORMAT(DATE_ADD(MAX(report_date), INTERVAL 1 MONTH), '%Y-%m-01')
FROM lingxing_ad_spend_daily
WHERE ad_type IN ('SP','SD');
") }"
...
SELECT DISTINCT CAST(a.store_name AS CHAR CHARACTER SET utf8mb4)
FROM lingxing_ad_spend_daily a
INNER JOIN ods_amz_all_orders_report o
  ON BINARY o.store_name = BINARY a.store_name
WHERE a.ad_type IN ('SP','SD')
...
while [[ "$month" < "$END_MONTH_EXCLUSIVE" ]]; do
  ...
  # 1. 广告需求：只使用SP和SD，排除__STORE__，按店铺+日期+SKU汇总。
  run_sql "[$month] 重建广告订单日需求" <<SQL
DELETE FROM dws_amz_ad_order_demand_daily
WHERE order_month = '$month';

INSERT INTO dws_amz_ad_order_demand_daily (
    store_name, report_date, order_month, sku_key, asin,
    sp_orders, sd_orders, ad_orders_raw,
    ad_sales, ad_cost, clicks, impressions,
...
SELECT
    ...
    COUNT(DISTINCT ad_type),
    COUNT(DISTINCT ad_type) > 1
FROM lingxing_ad_spend_daily
WHERE report_date >= '$month'
  AND report_date < '$next_month'
  AND ad_type IN ('SP','SD')
  AND orders > 0
  AND sku IS NOT NULL
  AND TRIM(sku) <> ''
...
INSERT INTO dwd_amz_order_attribution_base (
    source_id, store_name, order_date, order_month,
    purchase_date_utc, amazon_order_id, order_item_id,
    sku, sku_key, asin, quantity, currency,
    gross_item_sales, item_promotion_discount,
    net_item_sales, net_unit_price, promotion_ids,
    offsite_flag, shipping_promotion_flag,
...
SELECT
    ...
    n.currency,
    n.gross_item_sales,
    n.item_promotion_discount,
    n.net_item_sales,
    n.net_unit_price,
    n.promotion_ids,
    (n.promo_upper LIKE '%MPC-%') AS offsite_flag,
    (
       n.promo_upper LIKE '%FREE SHIPPING%'
       OR n.promo_upper LIKE '%A3JU1FCINF5SD0%'
    ) AS shipping_promotion_flag,
    (
...
FROM (
    SELECT
        ...
        CAST(COALESCE(order_item_id,'') AS CHAR CHARACTER SET utf8mb4) COLLATE utf8mb4_bin AS order_item_id,
        sku,
        ...
        (COALESCE(item_price,0) - COALESCE(item_promotion_discount,0)) / quantity AS net_unit_price,
        promotion_ids,
        UPPER(COALESCE(promotion_ids,'')) AS promo_upper
    FROM ods_amz_all_orders_report
    WHERE store_name = '$escaped_store'
      AND purchase_date_utc >= CONVERT_TZ('$month 00:00:00', 'America/Los_Angeles', 'UTC')
      AND purchase_date_utc <  CONVERT_TZ('$next_month 00:00:00', 'America/Los_Angeles', 'UTC')
...
INSERT INTO dwd_amz_ad_order_allocation (
    ...
    sp_orders, sd_orders, ad_orders_raw,
    allocated_ad_orders, unallocated_ad_orders,
    estimated_ad_flag
)
SELECT
    r.store_name,
...
INSERT INTO dwd_amz_order_attribution_result (
    source_id, store_name, order_date, order_month,
    amazon_order_id, order_item_id, sku, sku_key, asin,
    quantity, gross_item_sales, net_item_sales, net_unit_price,
    promotion_ids, offsite_flag, estimated_ad_flag,
    onsite_promotion_flag, low_price_flag,
    main_order_type, classification_reason,
    candidate_rank, ad_orders_raw
)
SELECT
    b.source_id,
    b.store_name,
    ...
    CASE
        ...
        WHEN COALESCE(a.estimated_ad_flag,0)=1 THEN '按店铺+当地日期+SKU统计分配SP/SD广告订单'
        WHEN b.onsite_promotion_flag=1 THEN 'Percentage Off、PLM或item_promotion_discount>0'
        WHEN b.low_price_candidate_flag=1 THEN '排除前三类后净成交单价<=10美元'
        ELSE '未命中站外、广告、站内促销或低价规则'
    END,
    a.candidate_rank,
    COALESCE(a.ad_orders_raw,0)
FROM dwd_amz_order_attribution_base b
...
INSERT INTO dws_amz_order_source_monthly (
    order_month, store_name, main_order_type,
    classified_item_rows, order_sku_count, amazon_order_count,
    units, gross_item_sales, net_item_sales
)
SELECT
    order_month,
    store_name,
...
SELECT
    ...
    ROUND(
        order_sku_count /
        SUM(order_sku_count) OVER (PARTITION BY order_month, store_name) * 100,
        2
    ) AS order_sku_pct,
    units,
    ROUND(net_item_sales,2) AS net_item_sales
